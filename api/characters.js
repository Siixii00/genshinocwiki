...
function getCharacterFields() {
  return [
    ...
    'avatar_position', 'avatar_scale', 'card_avatar_scale', 'card_avatar_position_y',
...
function normalizeCharacter(c) {
  return {
    ...
    card_avatar_scale: c.cardAvatarScale || c.card_avatar_scale || null,
    card_avatar_position_y: c.cardAvatarPositionY || c.card_avatar_position_y || null,
    ...
    constellation_bg_settings: (() => {
        const val = c.constellationBgSettings || c.constellation_bg_settings;
        if (!val) return null;
    ...
    if (req.method === 'POST') {
      ...
      const [character] = await sql`
        INSERT INTO characters (
          ...
          ${c.avatar_position}, ${c.avatar_scale}, ${c.card_avatar_scale}, ${c.card_avatar_position_y},
    ...
    if (req.method === 'PUT') {
      ...
      const [character] = await sql`
        UPDATE characters SET
          ...
          artwork = COALESCE(${c.artwork}, artwork),
          gacha_image = COALESCE(${c.gacha_image}, gacha_image),
          portrait = COALESCE(${c.portrait}, portrait),
          avatar = COALESCE(${c.avatar}, avatar),
          avatar_position = COALESCE(${c.avatar_position}, avatar_position),
          avatar_scale = COALESCE(${c.avatar_scale}, avatar_scale),
          card_avatar_scale = COALESCE(${c.card_avatar_scale}, card_avatar_scale),
          card_avatar_position_y = COALESCE(${c.card_avatar_position_y}, card_avatar_position_y),
          ...
          skill_normal_icon = COALESCE(${c.skill_normal_icon}, skill_normal_icon),
          ...
          skill_elemental_icon = COALESCE(${c.skill_elemental_icon}, skill_elemental_icon),
          ...
          skill_burst_icon = COALESCE(${c.skill_burst_icon}, skill_burst_icon),
          ...
          constellation_image = COALESCE(${c.constellation_image}, constellation_image),
          constellation_bg_settings = COALESCE(${c.constellation_bg_settings}, constellation_bg_settings),
